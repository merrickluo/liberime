...
static const char key_names[] = "space\0"
                                "exclam\0"
                                "quotedbl\0"
                                "numbersign\0"
                                "dollar\0"
...
int emacs_event_has_modifier(int event) {
  return event & (EMACS_CHAR_SHIFT | EMACS_CHAR_CTL | EMACS_CHAR_ALT |
                  EMACS_CHAR_SUPER | EMACS_CHAR_HYPER | EMACS_CHAR_META);
}

int emacs_int_event_to_key_sequence(int event, char *buf, size_t bufsz) {
  int modifiers =
      event & (EMACS_CHAR_SHIFT | EMACS_CHAR_CTL | EMACS_CHAR_ALT |
               EMACS_CHAR_SUPER | EMACS_CHAR_HYPER | EMACS_CHAR_META);
  int keycode = emacs_event_keycode(event);

  char tmp[256];
  tmp[0] = '\0';

  /* Handle Control characters (0-31): Emacs stores as raw ASCII without
   * modifier bits */
  int is_implicit_control = (keycode >= 0 && keycode <= 31);
  if (is_implicit_control) {
    /* Only add "Control+" if no explicit CTL modifier is already set */
    if (!(modifiers & EMACS_CHAR_CTL)) {
      strcat(tmp, "Control+");
    }
    /* Convert 0-31 to printable: 0->'@', 1->'a', 2->'b', ... 26->'z', etc. */
    if (keycode == 0) {
      keycode = '@'; /* C-@ */
    } else if (keycode >= 1 && keycode <= 26) {
      keycode = 'a' + keycode - 1; /* C-a -> 'a' */
    } else if (keycode >= 27 && keycode <= 31) {
      /* C-[ C-\ C-] C-^ C-_ */
      keycode = '[' + keycode - 27;
    }
  }

  /* Append explicit modifier names */
  int count = 0;
  if (modifiers & EMACS_CHAR_SHIFT) {
    strcat(tmp, "Shift+");
    count++;
  }
  if (modifiers & EMACS_CHAR_CTL) {
  ...
  if (modifiers & EMACS_CHAR_ALT) {
    strcat(tmp, "Alt+");
    count++;
  }
  if (modifiers & EMACS_CHAR_SUPER) {
    strcat(tmp, "Super+");
    count++;
  }
  if (modifiers & EMACS_CHAR_HYPER) {
    strcat(tmp, "Hyper+");
    count++;
  }
  if (modifiers & EMACS_CHAR_META) {
    strcat(tmp, "Meta+");
    count++;
  }
  ...
  if (keycode >= 0x20 && keycode <= 0x7e) {
    /* Printable ASCII */
    char final[256];
    int has_modifiers = (count > 0 || is_implicit_control);

    /* Braces are special: they're used for escaping in KeySequence::Parse,
     * so always use their names to avoid ambiguity */
    if (keycode == '{' || keycode == '}') {
      const char *name = rime_get_key_name(keycode);
      if (name) {
        if (has_modifiers) {
          snprintf(final, sizeof(final), "{%s%s}", tmp, name);
        } else {
          snprintf(final, sizeof(final), "{%s}", name);
        }
      } else {
        snprintf(final, sizeof(final), "%c", (char)keycode);
      }
    } else if (has_modifiers) {
      /* Has modifiers: use {Modifier+key} format */
      const char *name = rime_get_key_name(keycode);
      if (name) {
        snprintf(final, sizeof(final), "{%s%s}", tmp, name);
      } else {
        snprintf(final, sizeof(final), "{%s%c}", tmp, (char)keycode);
      }
    } else {
      /* No modifiers: output character directly, no braces needed */
      snprintf(final, sizeof(final), "%c", (char)keycode);
    }
    strncpy(tmp, final, sizeof(tmp) - 1);
    tmp[sizeof(tmp) - 1] = '\0';
  } else {
    /* Non-ASCII: look up key name */
    const char *name = rime_get_key_name(keycode);
    if (name) {
      /* Named key with modifiers: use {Modifier+Name} or {Name} format */
      char final[256];
      if (count > 0 || is_implicit_control) {
        snprintf(final, sizeof(final), "{%s%s}", tmp, name);
      } else {
        snprintf(final, sizeof(final), "{%s}", name);
      }
      ...
      /* Unknown key: output as hex */
      char hex[12];
      snprintf(hex, sizeof(hex), "0x%04x", keycode);
      strncat(tmp, hex, sizeof(tmp) - strlen(tmp) - 1);
    }
  }
...
int emacs_symbol_to_key_sequence(const char *sym_name, char *buf,
                                 size_t bufsz) {
  if (!sym_name || !buf || bufsz == 0)
    return -1;
  ...
  /* Parse modifier prefixes from symbol name.
     ...
     Modifier prefixes: C=Control, M=Meta, S=Shift, s=Super, H=Hyper, A=Alt */
  const char *key_part = sym_name;
  while (key_part[0] && key_part[1] == '-') {
    char mod = key_part[0];
    ...
    case 'A':
      ...
      break;
    default:
      goto no_more_mods; /* Unknown prefix, treat as key name */
    }
    if (tmp[0])
      strcat(tmp, "+");
    strcat(tmp, mod_name);
...
no_more_mods:
  ...
  if (!std_name) {
    /* Try to find the key by name (case-sensitive).
       key_names uses mixed case: "space" (lower), "Left" (capitalized).
       Try original first, then capitalized first letter. */
    int keycode = rime_get_keycode_by_name(key_part);

    /* Try capitalized version if original fails and starts with lowercase */
    char capitalized[64];
    if (keycode == XK_VoidSymbol && key_part[0] >= 'a' && key_part[0] <= 'z' &&
        strlen(key_part) < sizeof(capitalized)) {
      strncpy(capitalized, key_part, sizeof(capitalized) - 1);
      capitalized[sizeof(capitalized) - 1] = '\0';
      capitalized[0] = capitalized[0] - 'a' + 'A';
      keycode = rime_get_keycode_by_name(capitalized);
    }

    if (keycode != XK_VoidSymbol) {
      std_name = rime_get_key_name(keycode);
    }
  }

  if (std_name) {
    if (tmp[0]) {
      snprintf(buf, bufsz, "{%s+%s}", tmp, std_name);
    } else {
      snprintf(buf, bufsz, "{%s}", std_name);
    }
    return 0;
  }

  /* Not found: use symbol name as-is with braces */
  if (tmp[0]) {
    snprintf(buf, bufsz, "{%s+%s}", tmp, key_part);
  } else {
    snprintf(buf, bufsz, "{%s}", key_part);
  }
  return 0;
}
