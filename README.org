...
** 预编译（推荐）
可以在[[https://github.com/emacs-rime/liberime/releases][release]]中下载预编译好的二进制文件。注意，没有标明 ~-with-deps~ 的压缩包里均仅包含 ~liberime-core~ 的可执行文件。
*** Linux
可以下载 ~linux-aarch64~ ~linux-arm64~ ~linux-armhf~ ~linux-x86_64~ 架构的压缩包，提取 ~/lib~ 其中的 ~liberime-core.so~ 。需要将 ~liberime-module-file~ 指向 ~liberime-core.so~ 文件。
*** MacOS
可以下载 ~macos-arm64~ ~macos-x86_64~ 架构的压缩包，提取 ~/lib~ 其中的 ~liberime-core.dylib~ 。需要将 ~liberime-module-file~ 指向 ~liberime-core.dylib~ 文件。
*** Windows
如果你的 Emacs 位于 MSYS2 环境中，可以下载 ~clang-arm64~ ~clang-x86_64~ ~ucrt-x86_64~ ~x86_64~ 架构的压缩包，提取 ~/bin~ 其中的 ~liberime-core.dll~ 。需要将 ~liberime-module-file~ 指向 ~liberime-core.dll~ 文件。
